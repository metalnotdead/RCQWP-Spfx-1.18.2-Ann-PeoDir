...
export class PeopleDirectory extends React.Component<IPeopleDirectoryProps, IPeopleDirectoryState> {
  ...
  private _loadPeopleInfo(index: string, searchQuery: string): void {
    if(index==="Search" && searchQuery===null){
      this.setState({
        loading: false,
        errorMessage: null,
        people: []
      ...
      .then((res: IPeopleSearchResults): void => {
        ...
        if (this.state.searchQuery === '') {
          ...
          people = people.reduce((result: IPerson[], person: IPerson) => {
            if (
              (this.props.indexByLastname && person.lastName && person.lastName.indexOf(selectedIndex) === 0) || 
              (!this.props.indexByLastname && person.firstName && person.firstName.indexOf(selectedIndex) === 0)
            ) {
              result.push(person);
            }
            return result;
